    ...
    public class CategoriesHandlers : ICategoriesHandlers
    {
        ...
        public int CategoriePropertiesManage(Properties cp)
        {
            ...
            else
            {
                ...
                int rtn = MsSqlFactory.conn.m_ExecuteScalar(ref errText, "SELECT COUNT(1) FROM CategorieProperties WHERE id = " + cp.Id + "", ref s_cont);
        ...
        {
            string errText = string.Empty;
            int rtn = MsSqlFactory.conn.m_RunSQL(ref errText, "DELETE FROM ResourceProperties WHERE CategoriePropertieId='" + cpid + "''");
            rtn = MsSqlFactory.conn.m_RunSQL(ref errText, "DELETE FROM CategorieProperties WHERE id=" + cpid + "");
            return rtn;
        }
        ...
        public int DelCategories(string classid)
        {
            string s_cont = string.Empty;
            string errText = string.Empty;
            int rtn = MsSqlFactory.conn.m_ExecuteScalar(ref errText, "SELECT COUNT(1) FROM resources WHERE iClassID = '" + classid + "'", ref s_cont);
            ...
            rtn = MsSqlFactory.conn.m_ExecuteScalar(ref errText, "SELECT  COUNT(1) FROM Categories WHERE Parent = '" + classid + "'", ref s_cont);
            if (rtn < 0) return rtn;
