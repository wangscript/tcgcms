...
function InitDocument(hiddenid)
{
    ...
    if(body)
    {
        body.innerHTML=h.value;
    }
    else
    {
    	var WBTB_bodyTag="<html><head></head><BODY><style type='text/css'>body{font-size:12px; padding:3px;}</style>";
    	f.document.designMode="On"
        f.document.open();
        f.document.write(WBTB_bodyTag);
        if (h.value!="")
        {
	        f.document.write(h.value);
        }
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "ȫò༭Զִмв,ʹü̿ݼ(Ctrl+X)";
				break;
			case "Copy":
				sAlert = "ȫò༭Զִп,ʹü̿ݼ(Ctrl+C)";
				break;
			case "Paste":
				sAlert = "ȫò༭Զִճ,ʹü̿ݼ(Ctrl+V)";
				break;
		}
	}
	if(sAlert != ""){
...
function createLink() {
	var sURL=window.prompt("վַ:", "http://");
