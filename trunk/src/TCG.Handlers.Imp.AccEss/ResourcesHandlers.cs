...
namespace TCG.Handlers.Imp.AccEss
{
    public class ResourcesHandlers :IResourceHandlers
    {
        ...
        public int GetMaxResourceId()
        {
            ...
            int rtn = AccessFactory.conn.m_ExecuteScalar(ref errText, "SELECT max(id) FROM resources", ref s_cont);
        ...
        public DataTable GetResourcesList(string sqlsb)
        {
            string errText = string.Empty;
            DataSet ds = null;
            int rtn = AccessFactory.conn.m_RunSQLData(ref errText, sqlsb, ref ds);
            if (rtn < 0) return null;
            if (ds == null || ds.Tables.Count == 0) return null;
            return ds.Tables[0];
        }
        ...
        public int SaveOrDelResource(string ids, string action)
        {
            ...
            if (action == "SAVE")
            {
                ...
                rtn = AccessFactory.conn.m_RunSQL(ref errText, sql);
            }
        ...
        public int ResourcePropertiesManage(ResourceProperties cp)
        {
            ...
            else
            {
                ...
                string s_cont = string.Empty;
                int rtn = AccessFactory.conn.m_ExecuteScalar(ref errText, "SELECT COUNT(1) FROM ResourceProperties WHERE ResourceId = '"
